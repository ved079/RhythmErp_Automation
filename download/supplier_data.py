...
def generate_valid_step1_data(company_prefix="AutoSupplier"):
    ...
    return {
        ...
        "default_currency": None,      # Pick from live UI (REQUIRED)
...
def generate_valid_step2_data():
    ...
    Cascading dropdown values set to None — must be populated in order.
    """
    return {
        "address_type": None,          # Pick from live UI (REQUIRED): Shipping/Billing
        "country": None,               # Pick from live UI (REQUIRED) — cascading first
        "state": None,                 # Pick from live UI (REQUIRED, depends on Country)
        "district": None,              # Pick from live UI (REQUIRED, depends on State)
        "taluka": None,                # Pick from live UI (REQUIRED, depends on District)
        "village": None,               # Pick from live UI (optional, depends on Taluka)
...
def generate_valid_step3_data():
    ...
    return {
        ...
        "account_holder_name": generate_contact_person("Holder"),
...
def generate_valid_edit_data():
    ...
    return {
        ...
        "phone_number": generate_phone(),
        "pan_number": generate_pan(),
        "contact_person": generate_contact_person("Edit"),
        "office_number": generate_office_number(),
    }
...
def generate_duplicate_company_data(existing_company_name):
    ...
    return {
        "step1": {
            ...
            "is_gst_set_off": True,
            "is_tds_applicable": False,
            "contact_person": generate_contact_person("Dup"),
            "office_number": generate_office_number(),
            "payment_terms": None,
            "delivery_terms": None,
...
class ExpectedMessages:
    ...
    EMAIL_NO_VALIDATION = "(BUG-002: No email format validation)"
    # BUG: phone number has spinner controls
    PHONE_SPINNER_BUG = "(BUG-003: Phone Number type=number shows spinner controls)"
    # BUG: no PAN format validation
    PAN_NO_VALIDATION = "(BUG-004: No PAN format validation)"
...
class KnownBugs:
    ...
    BUG_002 = "BUG-002: No email format validation"
    BUG_003 = "BUG-003: Phone Number field shows spinner controls (type=number)"
    BUG_004 = "BUG-004: No PAN format validation"
    BUG_005 = "BUG-005: No Update button in Edit mode — cannot save edits"
