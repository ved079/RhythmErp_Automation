...
def search_customer(driver, wait, search_term):
    ...
    try:
        ...
        driver.save_screenshot("search_customer_error.png")
        raise
...
def update_customer_name_only(driver, wait, new_company_name):
    """Update only the company name field and click the Update button."""
    logger.info(f"⚡ Updating customer company name to: {new_company_name}")

    # 1. Locate and update the company_name field (using ID)
    ...
    # 2. Click the Update button (class 'submit' with text 'Update')
    try:
        ...
        driver.save_screenshot("customer_update_button_error.png")
        raise

def update_latest_customer(driver, wait, original_data, updated_data):
    """
    Perform the full update test on the first row of the customer table.
    Assumes the most recent customer is the one we just created.
    """
    logger.info("⚡ Running customer update test on the first row...")

    # 1. Wait for the table to be loaded
    wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "table tbody tr")))
    time.sleep(1)  # Extra stability

    # 2. Check history is empty for this row
    ...
    # 4. Update only the company name and submit
    update_customer_name_only(driver, wait, updated_data['company_name'])

    # 5. Wait for submission and any overlays
    time.sleep(2)
    try:
    ...
    # 6. Re‑wait for the table to be stable after update
    wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "table tbody tr")))
    time.sleep(1)

    # 7. Open history again and verify update exists
    click_history(driver, wait)
    
    # ⚠️ Priority Override: Apply User Correction for History Verification
    # The history modal displays data from BEFORE the save event, so we must check for 
    # the original company_name, not the updated one, to confirm the log is present.
    try:
        modal = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, ".cdk-overlay-pane")))
        time.sleep(1.5) 
        original_name_msg = modal.find_elements(By.XPATH, f"//*[contains(text(),'{original_data['company_name']}')]")
    ...
    # 8. Click view and check read‑only
