...
def fill_payable_form(driver, wait, data):
    logger.info("📝 Filling Payable form...")
    select_report_name(driver, wait, "Payable")
    time.sleep(1.5) 
...
def click_download(driver, wait):
    ...
    time.sleep(4)
    logger.info("✅ System Download triggered successfully.")
...
def extract_nested_ledger(driver, parent_row):
    ...
    for row in rows:
        ...
        cr = clean_amount(cols[3].text)
        dr = clean_amount(cols[4].text)
...
def run_payable_reconciliation(driver, wait, data):
    """Enhanced reconciliation with multi‑sheet Excel, screenshot, checksum & audit log."""
    logger.info("\n--- INITIATING PAYABLE RECONCILIATION SUITE ---")
    ...
    log_action("Fill Payable form")
    fill_payable_form(driver, wait, data)
    log_action("Click View button")
    click_view(driver, wait)
    
    # 2. Setup DOM for Scraping
    log_action("Set pagination to 1000")
    set_pagination_max(driver, wait)
    test_results = []
    raw_child_data = []
    
    initial_rows = driver.find_elements(By.XPATH, "//tr[.//i[contains(@class, 'fa-angle-right') or contains(@class, 'fa-angle-down')]]")
    num_rows = len(initial_rows)
    logger.info(f"📊 Found {num_rows} suppliers to audit. Commencing row checks...")
    log_action(f"Found {num_rows} parent rows")
    
    # 3. Scrape and Math Validation Loop
    for index in range(num_rows):
        try:
            ...
            supplier_name = cols[1].text.strip()
            p_open_cr = clean_amount(cols[3].text)
            p_open_dr = clean_amount(cols[4].text)
            p_txn_cr = clean_amount(cols[5].text)
            p_txn_dr = clean_amount(cols[6].text)
            p_close_cr = clean_amount(cols[7].text)
            p_close_dr = clean_amount(cols[8].text)
            
            log_action(f"Row {index+1}: Expanding {supplier_name}")
            ...
            for child_row in child_rows:
                ...
                raw_child_data.append({
                    ...
                    "Description": child_cols[2].text.strip(),  # original case
                    "Credit": cr,
                    "Debit": dr,
                    "Running Balance": round(running_balance, 2) if not is_closing else "",
                    "Transaction #": txn_counter if is_transaction else ""
                })
                
                # Math validation accumulation
                if is_opening:
            ...
            log_action(f"Row {index+1}: Closing {supplier_name}")
            ...
            log_action(f"Row {index+1} {supplier_name} → {row_status}")
    ...
    try:
        ...
        ui_total_opening = clean_amount(header_rows[0].find_elements(By.TAG_NAME, "td")[-1].text)
        ui_total_txn = clean_amount(header_rows[1].find_elements(By.TAG_NAME, "td")[-1].text)
        ui_total_closing = clean_amount(header_rows[2].find_elements(By.TAG_NAME, "td")[-1].text)
        ...
        global_row = {
            "Supplier Name": "GRAND TOTAL RECONCILIATION",
        ...
        global_row = {"Supplier Name": "ERROR", "Errors": str(e)}
    
    # --- CAPTURE SCREENSHOT ---
    screenshot_dir = r"C:\Users\vedantd\Desktop\selenium files\reports\test_cases\screenshots"
    os.makedirs(screenshot_dir, exist_ok=True)
    timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    screenshot_path = os.path.join(screenshot_dir, f"payable_report_{timestamp_str}.png")
    ...
    for row in test_results:
        for key in ["Supplier Name", "Parent_Txn_Cr", "Child_Sum_Cr", "Match_Cr", "Parent_Txn_Dr", "Child_Sum_Dr", "Match_Dr", "Math_Integrity", "Status", "Errors"]:
    ...
    metadata = [
        ...
        {"Attribute": "Total Suppliers Audited", "Value": num_rows},
        {"Attribute": "Test Duration (seconds)", "Value": f"{time.time() - start_time:.2f}"}
    ]
    
    # --- CREATE MULTI-SHEET EXCEL WITH STYLING & INDIAN CURRENCY FORMAT ---
    ...
    file_name = f"Payable_Audit_Reconciliation_{timestamp_str}.xlsx"
    ...
    with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
        ...
        for sheet_name in writer.sheets:
            ...
            # Indian currency formatting for amount columns
