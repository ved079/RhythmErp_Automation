...
def select_report_name(driver, wait, report_name="Profit Loss"):
...
def fill_profit_loss_form(driver, wait, data):
    logger.info("📝 Filling Profit Loss form...")
    
    if not select_report_name(driver, wait, "Profit Loss"):
        raise Exception("Could not select Report Name")
    
    time.sleep(1.5) 
    logger.info("   ⏳ Waiting for form to initialize...")
    
    # Wait for the first specific field of Profit Loss to load
    try:
        wait.until(EC.presence_of_element_located((By.ID, "transaction_date")))
    ...
    # 3. Smart Field Map for Profit Loss
    # Structure: (dict_key, html_locator, field_type, label_fallback)
    field_map = [
        ('transaction_date', 'transaction_date', 'input', None),
        ('level', 'level', 'dropdown', 'Level'),
        ('view_type', 'view_type', 'dropdown', 'View Type'),
        ('division', 'division_ref_id', 'dropdown', 'Division'),
        ('department', 'department_ref_id', 'dropdown', 'Department'),
        ('type_of_sale', 'sale_type_ref_id', 'dropdown', 'Type Of Sale'),
        ('location', 'location_ref_id', 'dropdown', 'Location'),
        ('file_format', 'file_format', 'dropdown', 'File Format'),
        ('amount_unit_conversion', 'unit_convertor', 'dropdown', 'Amount Unit Conversion')
    ]
    
    # 4. Loop through and route to the correct UI function
    for key, elem_locator, field_type, fallback_label in field_map:
        if data.get(key):
            if field_type == 'dropdown':
                try:
                    # 1st try: By ID
                    select_dropdown(driver, wait, value=data[key], control_id=elem_locator, searchable=True)
                except Exception:
                    try:
                        # 2nd try: By Form Control Name
                        select_dropdown(driver, wait, value=data[key], control_name=elem_locator, searchable=True)
                    except Exception:
                        try:
                            # 3rd try: By visible Label Text
                            logger.warning(f"   ⚠️ ID/Name failed for {elem_locator}, trying Label: '{fallback_label}'...")
                            select_dropdown(driver, wait, value=data[key], label_text=fallback_label, searchable=True)
                        except Exception:
                            # 4th try: Non-searchable fallback
                            logger.warning(f"   ⚠️ Searchable failed, trying non-searchable click for {elem_locator}...")
                            select_dropdown(driver, wait, value=data[key], control_id=elem_locator, searchable=False)
            
            elif field_type == 'input':
                time.sleep(1) 
                fill_input(driver, wait, value=data[key], control_id=elem_locator)
