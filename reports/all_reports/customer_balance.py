...
def select_report_name(driver, wait, report_name="Customer Balance"):
...
def fill_customer_balance_form(driver, wait, data):
    logger.info("📝 Filling Customer Balance form...")
    if not select_report_name(driver, wait, "Customer Balance"): raise Exception("Failed")
    time.sleep(1.5)
    try:
        wait.until(EC.presence_of_element_located((By.ID, "customer_ref_id")))
    ...
    field_map = [
        ('customer_name', 'customer_ref_id', 'dropdown', 'Customer Name', data.get('customer_name')),
