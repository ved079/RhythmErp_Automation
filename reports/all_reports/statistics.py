...
def fill_input(driver, wait, value, control_name=None, control_id=None):
    try:
        ...
        if control_name: element = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, f"input[formcontrolname='{control_name}']")))
        elif control_id: element = wait.until(EC.presence_of_element_located((By.ID, control_id)))
        else: raise ValueError("Either control_name or control_id must be provided")
...
def select_report_name(driver, wait, report_name="Statistics"):
...
def fill_statistics_form(driver, wait, data):
    logger.info("📝 Filling Statistics form...")
    if not select_report_name(driver, wait, "Statistics"): raise Exception("Failed")
    time.sleep(1.5)
    try:
        wait.until(EC.presence_of_element_located((By.ID, "transaction_from_date")))
    ...
    field_map = [
        ...
        ('from_date', 'transaction_from_date', 'input', 'From Date', data.get('from_date')),
        ('to_date', 'transaction_to_date', 'input', 'To Date', data.get('to_date'))
    ]
    
    for key, elem_locator, field_type, fallback_label, val in field_map:
        if val: 
            if field_type == 'dropdown':
                try: select_dropdown(driver, wait, value=val, control_id=elem_locator, searchable=False)
                except: select_dropdown(driver, wait, value=val, control_name=elem_locator, searchable=False)
            elif field_type == 'input':
                time.sleep(1); fill_input(driver, wait, value=val, control_id=elem_locator)
...
def run(driver, wait, data):
    ...
    fill_statistics_form(driver, wait, data)
