...
def fill_inventory_report_form(driver, wait, data):
    logger.info("📝 Filling Inventory Report form...")
    
    if not select_report_name(driver, wait, "Inventory Report"):
        raise Exception("Could not select Report Name")
    
    time.sleep(1.5) 
    logger.info("   ⏳ Waiting for form to initialize...")
    
    # Wait for the first specific field to load
    try:
        wait.until(EC.presence_of_element_located((By.ID, "item_ref_id")))
    ...
    # BACKWARDS COMPATIBILITY: If passed your purchase flow data list, grab the first item
    item_value = data.get('item')
    if not item_value and 'items' in data and len(data['items']) > 0:
        item_value = data['items'][0]['item']
    
    # 3. Smart Field Map
    # Structure: (dict_key, html_locator, field_type, label_fallback, actual_value)
    field_map = [
        ('item', 'item_ref_id', 'dropdown', 'Item', item_value),
        ('from_date', 'from_date', 'input', None, data.get('from_date')),
        ('to_date', 'to_date', 'input', None, data.get('to_date')),
    ...
    for key, elem_locator, field_type, fallback_label, val in field_map:
        if val: # Only try to fill if we actually have data for it
...
def fill_recon_inventory_report_form(driver, wait, data):
    logger.info("📝 Filling Inventory Report form for Recon Test...")
    ...
    field_map = [
        ('item', 'item_ref_id', 'dropdown', 'Item', data.get('item')),
        ('to_date', 'to_date', 'input', None, data.get('to_date')),
        ('division', 'division_ref_id', 'dropdown', 'Division', data.get('division')),
        ('department', 'department_ref_id', 'dropdown', 'Department', data.get('department')),
        ('type_of_sale', 'sale_type_ref_id', 'dropdown', 'Type Of Sale', data.get('type_of_sale')),
    ...
    # --- CRITICAL: Click View to generate the report ---
    view_btn = wait.until(EC.element_to_be_clickable((By.XPATH, "//button[contains(.,'View')]")))
    driver.execute_script("arguments[0].click();", view_btn)
    ...
    logger.info("✅ Recon Form filling complete and report loaded.")

def load_recon_report_data(driver, wait, data):
    """
    The streamlined orchestrator function for the Recon test.
    Navigates, fills the minimal form, and clicks View. 
    """
    go_to_reports_page(driver, wait)
    fill_recon_inventory_report_form(driver, wait, data)
    click_view(driver, wait)

def run(driver, wait, data):
    """Complete Inventory Report flow."""
    go_to_reports_page(driver, wait)
