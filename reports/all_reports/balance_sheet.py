...
def select_report_name(driver, wait, report_name="Balance Sheet"):
...
def fill_balance_sheet_form(driver, wait, data):
    logger.info("📝 Filling Balance Sheet form...")
    
    # 1. Select the main Report Name
    if not select_report_name(driver, wait, "Balance Sheet"):
        raise Exception("Could not select Report Name")
    
    time.sleep(1.5) 
    logger.info("   ⏳ Waiting for level dropdown to appear...")
    
    # Wait for the first specific field of the Balance Sheet to load
    try:
        wait.until(EC.presence_of_element_located((By.ID, "level")))
    ...
    # 3. Smart Field Map based strictly on the Balance Sheet screenshot
    field_map = [
        ('level', 'level', 'dropdown'),
        ('view_type', 'view_type', 'dropdown'),
        ('file_format', 'file_format', 'dropdown'),
        ('amount_unit_conversion', 'unit_convertor', 'dropdown'),
