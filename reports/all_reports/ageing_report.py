...
def select_report_name(driver, wait, report_name="Ageing Report"):
...
def fill_ageing_report_form(driver, wait, data):
    logger.info("📝 Filling Ageing Report form...")
    
    if not select_report_name(driver, wait, "Ageing Report"):
        raise Exception("Could not select Report Name")
    
    time.sleep(1.5) 
    logger.info("   ⏳ Waiting for form to initialize...")
    
    # Wait for transaction_type to load
    try:
        wait.until(EC.presence_of_element_located((By.ID, "transaction_type")))
    ...
    # Smart Field Map for Ageing Report
    field_map = [
        ('transaction_type', 'transaction_type', 'dropdown', 'Transaction Type', data.get('transaction_type')),
        ('due_status', 'due_status', 'dropdown', 'Due Status', data.get('due_status')),
        ...
        ('group_by', 'group_by', 'dropdown', 'Group By', data.get('group_by'))
    ]
            ...
            if field_type == 'dropdown':
                try:
                    ...
                    try:
                        ...
                        try:
                            logger.info(f"   ⚠️ ID/Name failed for {elem_locator}, trying Label: '{fallback_label}'...")
                            select_dropdown(driver, wait, value=val, label_text=fallback_label, searchable=True)
                        except Exception:
                            logger.info(f"   ⚠️ Searchable failed, trying non-searchable click for {elem_locator}...")
...
def run(driver, wait, data):
    """Complete Ageing Report flow."""
    nav_section.go_to_reports_page(driver, wait)
