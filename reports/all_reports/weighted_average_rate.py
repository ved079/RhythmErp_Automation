...
def select_report_name(driver, wait, report_name="Weighted Average Rate Report"):
...
def fill_weighted_avg_rate_form(driver, wait, data):
    logger.info("📝 Filling Weighted Average Rate form...")
    if not select_report_name(driver, wait, "Weighted Average Rate Report"): raise Exception("Failed")
    time.sleep(1.5)
    try:
        wait.until(EC.presence_of_element_located((By.ID, "to_date")))
    ...
    field_map = [
        ...
        ('for_day', 'to_date', 'input', 'For Day', data.get('for_day'))
    ]
