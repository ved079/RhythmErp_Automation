...
def select_report_name(driver, wait, report_name="Day Book"):
...
def fill_day_book_form(driver, wait, data):
    logger.info("📝 Filling Day Book form...")
    if not select_report_name(driver, wait, "Day Book"): raise Exception("Failed to select report")
    time.sleep(1.5) 
    logger.info("   ⏳ Waiting for form to initialize...")
    try:
        wait.until(EC.presence_of_element_located((By.ID, "for_type")))
    ...
    field_map = [
        ('for_type', 'for_type', 'dropdown', 'For Type', data.get('for_type')),
        ('frequency', 'frequency', 'dropdown', 'Frequency', data.get('frequency')),
        ('voucher_type', 'voucher_type', 'dropdown', 'Voucher Type', data.get('voucher_type')),
