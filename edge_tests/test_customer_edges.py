...
# ─────────────────────────────────────────────
SCREENSHOT_DIR = os.path.join(os.path.dirname(__file__), "screenshots", "customer")
...
def do_setup_and_navigate(driver, wait):
    """Navigate to customer page. Used by runner.py AND the pytest fixture below."""
...
class TestCustomerEdgeCases:
    ...
    def test_customer_empty_form_errors(self, driver, wait):
        """Clicking Submit on an empty Customer form should show validation errors for all mandatory fields."""
        ...
        logger.info(f"[DEBUG] Empty customer form errors: {error_texts}")
        ...
        assert "supply" in error_block or "type" in error_block, \
            f"Missing validation for Supply/Customer Type! Errors found: [{error_block}]"

    def test_customer_phone_9_digits_rejected(self, driver, wait):
    ...
    def test_customer_phone_10_digits_accepted(self, driver, wait):
    ...
    def test_customer_phone_11_digits_rejected(self, driver, wait):
    ...
    def test_customer_pan_rejects_invalid_format(self, driver, wait):
        """PAN with wrong format (numbers instead of letters) should be rejected."""
        pan_input = wait.until(EC.visibility_of_element_located((By.ID, "pan_no")))
        pan_input.clear()
        pan_input.send_keys("12345ABCDE")
    ...
    def test_customer_pan_valid_format_no_error(self, driver, wait):
        """PAN in valid format (ABCDE1234F) should NOT show a PAN-specific validation error."""
        pan_input = wait.until(EC.visibility_of_element_located((By.ID, "pan_no")))
        pan_input.clear()
    ...
    def test_customer_deposit_rejects_letters(self, driver, wait):
        """Deposit field should reject alphabetical characters."""
        ...
        deposit_input = wait.until(EC.visibility_of_element_located((By.ID, "deposit")))
        deposit_input.clear()
        ...
        logger.info(f"[DEBUG] Invalid deposit errors: {error_texts}")

        assert "deposit" in error_block or "number" in error_block or "numeric" in error_block, \
            f"BUG: Letters accepted in Deposit! Errors found: [{error_block}]"

    def test_customer_reset_button_clears_form(self, driver, wait):
