"""
conftest.py — Register Charges Screen (RhythmERP)
"""
...
def driver():
    log.separator()
    log.info("LAUNCHING BROWSER (RhythmERP - Register Charges Tests)...")
    log.separator()
    drv = get_driver()
    drv.maximize_window()
    yield drv
...
def rc_page(logged_in_driver):
    from pages.registration.modules.register_charges.register_charges_page import (
        RegisterChargesPage,
    )

    page = RegisterChargesPage(logged_in_driver)
