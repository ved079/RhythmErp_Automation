"""
test_employee_perf.py — Performance benchmarks for Employee API.
...
class TestEmployeePerformance:
    """Performance benchmarks for Employee operations."""

    def test_batch_create_5_employees(self, api_client):
        """Create 5 employees via API — should complete in under 10s."""
        start = time.time()
        for _ in range(5):
        ...
        assert elapsed < 10, f"5 employees took {elapsed:.1f}s (expected < 10s)"
