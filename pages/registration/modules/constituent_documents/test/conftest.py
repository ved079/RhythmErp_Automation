"""
conftest.py — Constituent Documents Screen (RhythmERP)
"""
...
def driver():
    log.separator()
    log.info("LAUNCHING BROWSER (RhythmERP - Constituent Documents Tests)...")
    log.separator()
    drv = get_driver()
    drv.maximize_window()
    yield drv
...
def cd_page(logged_in_driver):
    from pages.registration.modules.constituent_documents.constituent_documents_page import (
        ConstituentDocumentsPage,
    )

    page = ConstituentDocumentsPage(logged_in_driver)
