...
from pages.registration.modules.supplier.supplier_page import SupplierPage
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "search: Search and listing tests")
    config.addinivalue_line("markers", "row_actions: View / Edit / History row action tests")
...
def supplier_page(logged_in_page):
    p = SupplierPage(logged_in_page)
    p.navigate_to_page()
    yield p
    try:
        p.close_popup()
