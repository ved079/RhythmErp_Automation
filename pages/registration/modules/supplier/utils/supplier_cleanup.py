...
supplier_cleanup.py
-------------------
No-delete cleanup strategy for Supplier module tests.
...
NEVER implement delete_supplier() or cleanup_all() in this module.
"""
...
@dataclass
class CreatedRecord:
    """Represents a single created supplier record for tracking."""
    id: Optional[int]
    company_name: str
    timestamp: str = ""
    prefix: str = ""
...
class CleanupTracker:
    """
    Tracks all supplier records created during a test session.
    ...
    Usage:
        ...
        tracker.track(id=123, company_name="AutoSup_20260609_abc12345")
        ...
        # At session end:
    ...
    def generate_reports(
        ...
        Args:
            output_dir: Directory for report files. If None, auto-generates
                        a path under the supplier module's reports/cleanup/ dir.
        ...
        json_path = os.path.join(output_dir, f"supplier_cleanup_{timestamp}.json")
        csv_path = os.path.join(output_dir, f"supplier_cleanup_{timestamp}.csv")
