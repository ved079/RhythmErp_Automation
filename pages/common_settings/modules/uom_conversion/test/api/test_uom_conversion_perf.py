"""
test_uom_conversion_perf.py — Performance benchmarks for UOM Conversion payload generation.
...
class TestUOMConversionPerformance:
    """Verify UOM Conversion payload generation meets speed benchmarks."""

    def test_payload_generation_speed(self):
        """Single payload generation should complete in under 10ms."""
        start = time.perf_counter()
        for _ in range(100):
            generate_uom_conversion_api_payloads(count=1)
    ...
    def test_batch_create_5_conversions(self, api_client):
        """Create 5 UOM Conversions via API and verify they are persisted."""
        pass
