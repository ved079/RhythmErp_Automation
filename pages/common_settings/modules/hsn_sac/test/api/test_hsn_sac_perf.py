"""
test_hsn_sac_perf.py — Performance benchmarks + Live CRUD tests.
...
class TestHsnSacPerformance:
    """Verify HSN SAC payload generation meets speed benchmarks and live CRUD."""
    ...
    def test_batch_create_5_hsn_sacs(self, api_client):
        ...
        for i, p in enumerate(payloads):
            ...
            p["hsn_sac_description"] = f"{p.get('hsn_sac_description', '')}-{ts}{i}"
        ...
        for i, entry_id in enumerate(created_ids):
            ...
            current_val = detail.get("hsn_sac_description", "") or ""
            update_payload = dict(detail)
            update_payload["hsn_sac_description"] = current_val + "-UPD"
