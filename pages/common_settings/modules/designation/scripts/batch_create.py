...
batch_create.py
---------------
Create multiple Designation entries via API with realistic Indian job titles.

Just paste your Bearer token and go.

Usage:
    python pages/common_settings/modules/designation/scripts/batch_create.py
    python pages/common_settings/modules/designation/scripts/batch_create.py --count 20
    python pages/common_settings/modules/designation/scripts/batch_create.py --token eyJhbGci...
"""
...
# Add project root to path (designation/scripts → designation → modules → common_settings → pages → project root)
...
def batch_create(client, count, dry_run=False):
    ...
    print("=" * 70)
    print(f"  DESIGNATION BATCH CREATE — {count} entries")
    print("=" * 70)
...
def main():
    ...
    if not token:
        print("=" * 70)
        print("  DESIGNATION BATCH CREATE")
        print("=" * 70)
        print()
    ...
    result = client.list_entries("Designation", page=1, page_size=1)
    if not result:
