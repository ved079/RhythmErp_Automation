...
def vehicle_master_page(logged_in_driver):
    """Vehicle Master page object — optimized for speed.
    Setup: navigate only if not already on page (teardown already refreshed).
    Teardown: hard_refresh after each test for clean state (user requirement)."""
    ...
    if "Vehicle%20Master" in current_url or "Vehicle Master" in current_url:
        # Already on page — just verify it's ready (skip hard_refresh, teardown already did it)
        try:
            if not page.is_page_loaded():
