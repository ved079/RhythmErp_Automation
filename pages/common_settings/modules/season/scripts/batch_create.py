#!/usr/bin/env python3
"""
Season — Batch Create via API

Flat screen with no FK dropdowns.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Batch create Season entries via API")
    ...
    parser.add_argument("--offset", type=int, default=0, help="Start index in data pool (ignored for Season)")
...
def main():
    ...
    reset_season_name_pool()
    try:
        payloads = generate_season_api_payloads(count=count)
