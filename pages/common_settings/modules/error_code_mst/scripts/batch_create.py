#!/usr/bin/env python3
"""
Error Code Mst — Batch Create via API

Flat screen with 1 FK dropdown (error_code_type).
...
FK_SCREEN_MAP = {
    "error_code_type": "Error Code Type",
}
...
def resolve_all_fk_ids(resolver):
    """Resolve all Error Code Mst FK IDs from the live ERP."""
    fk_ids = {}
...
def main():
    ...
    try:
        payloads = generate_error_code_mst_api_payloads(count=count, offset=args.offset, fk_ids={})
    except Exception as e:
    ...
    if args.dry_run:
        ...
        for j, p in enumerate(payloads):
            name = f"code={p.get('code','')[:30]} type={p.get('error_code_type','')}"
            print(f"    [{j+1}] {name}")
        return
    ...
    if "error_code_type" not in fk_ids or not fk_ids["error_code_type"]:
        for attempt in ["Error Code", "Error Code Master"]:
            resolved = resolver.resolve(attempt)
            if resolved:
                print(f"    error_code_type: Found {len(resolved)} values from '{attempt}'")
                fk_ids["error_code_type"] = resolved
                break
    ...
    try:
        payloads = generate_error_code_mst_api_payloads(count=count, offset=args.offset, fk_ids=fk_ids)
    except Exception as e:
        print(f"  ERROR generating payloads: {e}")
        api.close()
        return
    ...
    for i, p in enumerate(payloads):
        missing = []
        if p.get("error_code_type") is None:
            missing.append("error_code_type")
        if missing:
