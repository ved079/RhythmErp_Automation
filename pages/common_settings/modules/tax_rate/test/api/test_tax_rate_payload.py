...
from pages.common_settings.modules.tax_rate.data.tax_rate_data import (
    ...
    generate_batch_payloads, TAX_TYPE_IDS, TAX_AUTHORITY_IDS,
    HSN_SAC_NUMBER_IDS, DEFAULT_TAX_RATE_FK_IDS, STEPPER_NAME,
)

@pytest.mark.api
class TestTaxRateAPIPayload:
    def test_payload_has_required_keys(self):
        p = generate_tax_rate_api_payloads(count=1)[0]
    ...
    def test_payload_attribute_name(self):
        assert generate_tax_rate_api_payloads(count=1)[0]["attribute_name"] == "Tax Rate"

    def test_payload_id_is_empty(self):
        assert generate_tax_rate_api_payloads(count=1)[0]["id"] == ""

    def test_payload_tax_rate_name_is_string(self):
        assert isinstance(generate_tax_rate_api_payloads(count=1)[0]["tax_rate_name"], str)

    def test_payload_tax_type_is_integer(self):
        assert isinstance(generate_tax_rate_api_payloads(count=1)[0]["tax_type_ref_id"], int)

    def test_payload_tax_authority_is_integer(self):
        assert isinstance(generate_tax_rate_api_payloads(count=1)[0]["tax_authority_ref_id"], int)

    def test_payload_from_date_is_string(self):
        assert isinstance(generate_tax_rate_api_payloads(count=1)[0]["from_date"], str)

    def test_payload_to_date_is_string(self):
        assert isinstance(generate_tax_rate_api_payloads(count=1)[0]["to_date"], str)

    def test_payload_revision_status_is_active(self):
        assert generate_tax_rate_api_payloads(count=1)[0]["revision_status"] == "Active"

    def test_payload_tax_type_in_valid_pool(self):
        valid = set(TAX_TYPE_IDS.values())
        for p in generate_tax_rate_api_payloads(count=5):
            assert p["tax_type_ref_id"] in valid

    def test_payload_tax_authority_in_valid_pool(self):
        valid = set(TAX_AUTHORITY_IDS.values())
        for p in generate_tax_rate_api_payloads(count=5):
    ...
    def test_payload_stepper_hsn_sac_in_valid_pool(self):
        valid = set(HSN_SAC_NUMBER_IDS.values())
        p = generate_tax_rate_api_payloads(count=1)[0]
...
class TestTaxRateBatchGeneration:
    ...
    def test_batch_all_fk_valid(self):
        ...
        va = set(TAX_AUTHORITY_IDS.values())
        for p in generate_batch_payloads(count=10):
