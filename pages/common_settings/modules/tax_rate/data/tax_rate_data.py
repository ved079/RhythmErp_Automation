...
VERSION_BUTTON_TEXT = "Version"
TAX_TYPE_OPTIONS = list(TAX_TYPE_IDS.keys())
...
def generate_revision_status():
    """Return a revision status string."""
    return "Active"


def generate_tax_rate_value():
    """Return a tax rate value (float)."""
    return random.choice([0.0, 0.25, 3.0, 5.0, 12.0, 18.0, 28.0])


def generate_valid_tax_rate_data():
    """Return a dict with header + sub_table_rows for UI tests."""
    return {
        "header": {
            "tax_rate_name": generate_tax_rate_name(),
            "tax_type": random.choice(TAX_TYPE_OPTIONS),
...
def generate_sub_table_row():
    """Return a single sub-table row dict with hsn_number and tax_rate."""
    return {
        "hsn_number": random.choice(list(HSN_SAC_NUMBER_IDS.keys())),
        "tax_rate": generate_tax_rate_value(),
    }


def generate_create_test_data():
    """Return a dict with header + sub_table_rows for create_record."""
    return generate_valid_tax_rate_data()


def generate_create_multi_row_data(row_count=3):
    """Return a dict with header + multiple sub_table_rows."""
    data = generate_valid_tax_rate_data()
    data["sub_table_rows"] = [generate_sub_table_row() for _ in range(row_count)]
    return data


def generate_version_test_data():
    """Return a dict for version testing."""
    return generate_valid_tax_rate_data()


def empty_fields_data():
    """Return a dict with all header fields empty."""
    return {
        "header": {
            "tax_rate_name": "",
            "tax_type": "",
...
def missing_name_data():
    """Return a dict with tax_rate_name empty."""
    return {
        "header": {
            "tax_rate_name": "",
            "tax_type": random.choice(TAX_TYPE_OPTIONS),
...
def missing_tax_type_data():
    """Return a dict with tax_type empty."""
    return {
        "header": {
            "tax_rate_name": generate_tax_rate_name(),
            "tax_type": "",
...
def missing_tax_authority_data():
    """Return a dict with tax_authority empty."""
    return {
        "header": {
            "tax_rate_name": generate_tax_rate_name(),
            "tax_type": random.choice(TAX_TYPE_OPTIONS),
...
def missing_revision_status_data():
    """Return a dict with revision_status empty."""
    return {
        "header": {
            "tax_rate_name": generate_tax_rate_name(),
            "tax_type": random.choice(TAX_TYPE_OPTIONS),
...
def sql_injection_name_data():
    """Return a dict with SQL injection in tax_rate_name."""
    data = generate_valid_tax_rate_data()
    data["header"]["tax_rate_name"] = "' OR 1=1; --"
    return data


def special_chars_name_data():
    """Return a dict with special characters in tax_rate_name."""
    data = generate_valid_tax_rate_data()
    data["header"]["tax_rate_name"] = f"TEST@#$%{generate_tax_rate_name()}"
    return data


def very_long_name_data():
    """Return a dict with a very long tax_rate_name (256 chars)."""
    data = generate_valid_tax_rate_data()
    data["header"]["tax_rate_name"] = "A" * 256
    return data


def negative_tax_rate_data():
    """Return a dict with a negative tax rate value in sub-table."""
    data = generate_valid_tax_rate_data()
    data["sub_table_rows"] = [{"hsn_number": random.choice(list(HSN_SAC_NUMBER_IDS.keys())), "tax_rate": -5.0}]
    return data


def zero_tax_rate_data():
    """Return a dict with a zero tax rate value in sub-table."""
    data = generate_valid_tax_rate_data()
    data["sub_table_rows"] = [{"hsn_number": random.choice(list(HSN_SAC_NUMBER_IDS.keys())), "tax_rate": 0}]
    return data


def very_large_tax_rate_data():
    """Return a dict with a very large tax rate value in sub-table."""
    data = generate_valid_tax_rate_data()
    data["sub_table_rows"] = [{"hsn_number": random.choice(list(HSN_SAC_NUMBER_IDS.keys())), "tax_rate": 999999}]
    return data


def empty_sub_table_data():
    """Return a dict with no sub-table rows."""
    data = generate_valid_tax_rate_data()
    data["sub_table_rows"] = []
    return data


def unselected_hsn_data():
    """Return a dict with unselected (empty) HSN in sub-table."""
    data = generate_valid_tax_rate_data()
