...
def hard_refresh_every_test(bank_page):
    ...
    # BEFORE: navigate to bank page + lightweight refresh
    bank_page.navigate_to_bank()
    bank_page.driver.refresh()
    bank_page.wait_seconds(1)
    yield
    # AFTER: recover from stuck state + lightweight refresh for next test
...
class TestBankEditFlow:
    ...
    def test_08_edit_updates_bank_name(self, bank_page):
        ...
        bank_page.set_is_default_bank(data.get(FIELD_IS_DEFAULT_BANK, "No"))
        bank_page.set_status(data.get(FIELD_STATUS, "Active"))
...
class TestBankDropdowns:
    ...
    def test_16_account_type_has_two_options(self, bank_page):
        ...
        # Step 1: Open Add form + open dropdown with hard refresh retry
        ...
        for attempt in range(3):
            ...
            # Use _open_dropdown (has 3-strategy click + hard refresh recovery)
            ...
            # Dropdown didn't open — close form, loop will re-open
            log.warning(f"T16: Account Type dropdown failed on attempt {attempt+1}, retrying...")
            try:
                bank_page.close_form_via_cancel()
            except Exception:
                pass
        ...
        # Step 3: Select Current via JS click
        try:
    ...
    def test_17_gl_account_searchable(self, bank_page):
        ...
        for attempt in range(3):
            ...
            # Dropdown didn't open — close form, loop will re-open
            log.warning(f"T17: GL Account dropdown failed on attempt {attempt+1}, retrying...")
            try:
                bank_page.close_form_via_cancel()
            except Exception:
                pass
        ...
        # Step 3: Verify filtered options contain "Cash"
        options = bank_page.driver.find_elements(
            By.CSS_SELECTOR, "mat-option"
        )
        ...
        # Step 4: Select first option via JS click
        try:
...
class TestBankGLAccountSearch:
    ...
    def test_30_gl_account_search_cash_keyword(self, bank_page):
        ...
        for attempt in range(3):
            ...
            # Dropdown didn't open — close form, loop will re-open
            log.warning(f"T30: GL Account dropdown failed on attempt {attempt+1}, retrying...")
            try:
                bank_page.close_form_via_cancel()
            except Exception:
                pass
        ...
        # Step 4: Select first matching option via JS click
        try:
            first_option = ("xpath", "//mat-option[contains(@class,'mat-mdc-option')][1]")
            el = bank_page.find_element(first_option)
            bank_page.driver.execute_script("arguments[0].click();", el)
