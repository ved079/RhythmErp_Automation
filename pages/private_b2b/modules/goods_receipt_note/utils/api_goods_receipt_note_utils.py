...
from pages.private_b2b.modules.goods_receipt_note.api.endpoints import (
    SCREEN_NAME,
    build_create_url,
    build_get_url,
    build_list_url,
...
class GRNAPIUtils:
    ...
    def create_grn(self, payload: dict = None, **overrides) -> Optional[Dict]:
        if payload is None:
            payload = generate_grn_payload(fk_overrides=overrides)
    ...
    def get_grn(self, entry_id) -> Optional[Dict]:
    ...
    def list_grns(self, page: int = 1, page_size: int = 20) -> Optional[Dict]:
    ...
    def update_grn(self, entry_id: int, payload: dict) -> Optional[Dict]:
    ...
    def create_and_verify_calculations(self, payload: dict = None, **overrides) -> Dict:
        ...
        result = self.create_grn(payload=payload)
        assert result is not None, f"GRN creation failed (status {self._last_status})"

        entry_id = result.get("id") or result.get("entry_id")
        assert entry_id is not None, "No entry ID in create response"

        entry = self.get_grn(entry_id)
        assert entry is not None, f"Failed to fetch GRN {entry_id}"

        assert entry.get("id") == entry_id, "Entry ID mismatch"
