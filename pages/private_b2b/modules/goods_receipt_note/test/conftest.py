...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "update: GRN update/edit tests")


@pytest.fixture(scope="session")
def driver():
    log.separator()
    log.info("LAUNCHING BROWSER (RhythmERP - GRN Tests)...")
    log.separator()
...
@pytest.fixture
def grn_page(logged_in_driver):
    from pages.private_b2b.modules.goods_receipt_note.goods_receipt_note_page import GoodsReceiptNotePage
    page = GoodsReceiptNotePage(logged_in_driver)
    try:
...
def grn_api(erp_api):
    ...
    return GRNAPIUtils(api_client=erp_api)
