...
@pytest.mark.smoke
class TestPOGPGRNQCPBBatchFlow:
    """Batch mode for the general PO→GP→GRN→QC→PB flow.

    Uses FLOW_CONFIGS env var (set by FastAPI). Each test method loops all N chains
    for that document type. Item is resolved dynamically from CBR (any qualifying item).
    """

    def test_batch_po(self, po_page, flow_state, flow_configs):
        flow_state["flow_configs"] = flow_configs
        flow_state["po_refs"] = []
        for i, cfg in enumerate(flow_configs):
            if i > 0:
                _hard_refresh(po_page)
            prev_top = po_page.get_ref_no_of_first_row()
            print(f"PRE_TOP:PO:{prev_top}", flush=True)
    ...
    def test_batch_gp(self, gp_page, flow_state):
        flow_state["gp_refs"] = []
        for i, cfg in enumerate(flow_state["flow_configs"]):
            if i > 0:
                _hard_refresh(gp_page)
            prev_top = gp_page.get_ref_no_of_first_row()
            print(f"PRE_TOP:GP:{prev_top}", flush=True)
    ...
    def test_batch_pb(self, pb_page, flow_state):
        for i in range(len(flow_state["flow_configs"])):
            if i > 0:
                _hard_refresh(pb_page)
            prev_top = pb_page.get_ref_no_of_first_row()
            print(f"PRE_TOP:PB:{prev_top}", flush=True)
