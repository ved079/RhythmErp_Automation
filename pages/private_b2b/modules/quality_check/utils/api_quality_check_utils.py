...
from pages.private_b2b.modules.quality_check.api.endpoints import (
    SCREEN_NAME,
    build_create_url,
    build_get_url,
    build_list_url,
...
class QCAPIUtils:
    ...
    def create_qc(self, payload: dict = None, **overrides) -> Optional[Dict]:
        if payload is None:
    ...
    def create_and_verify_calculations(self, payload: dict = None, **overrides) -> Dict:
        ...
        result = self.create_qc(payload=payload)
        assert result is not None, f"QC creation failed (status {self._last_status})"

        entry_id = result.get("id") or result.get("entry_id")
        assert entry_id is not None, "No entry ID in create response"

        entry = self.get_qc(entry_id)
        assert entry is not None, f"Failed to fetch QC {entry_id}"
