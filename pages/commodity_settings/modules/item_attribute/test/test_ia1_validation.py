"""
Item Attribute 1 (IA1) — 23 Automated Test Cases

IA1 is UNIQUE among the 5 IA screens: it has a Base UOM (mat-select dropdown, required).
Fields: Name (required), Base UOM (required dropdown), Description (optional), Status (toggle).

6 Test Classes:
  TestCreateFormValidations   (10 tests: IA1-C01 to IA1-C10)
  TestEditFormValidations     (4 tests:  IA1-E01 to IA1-E04)
  TestSearchFilter            (4 tests:  IA1-S01 to IA1-S04)
  TestPopupUIBehaviors        (3 tests:  IA1-P01 to IA1-P03)
  TestHistoryValidations      (2 tests:  IA1-H01 to IA1-H02)
...
GOLDEN CODE pattern (UOM v2):
  ...
  - Hardcoded ATTR_NUM = 1 (no parametrization — enables xdist -n 5)
"""
...
# ╔══════════════════════════════════════════╗
# ║  PHASE 1: Create Form Validations (10)  ║
# ╚══════════════════════════════════════════╝

class TestCreateFormValidations:
    """IA1-C01 to IA1-C10: Create form validation tests for IA1."""

    @pytest.mark.smoke
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_empty_form_submit(self, ia_page):
        """IA1-C01: Submit with all fields empty -> Validation Failed."""
        try:
    ...
    def test_only_name_filled(self, ia_page):
        """IA1-C02: Submit with only Name filled (no Base UOM) -> Should fail.
        IA1 requires Base UOM — unlike IA2-5 which pass with just Name."""
        try:
            data = {
                'name': generate_ia_name("NameOnly", ATTR_NUM),
                'description': None,
            }
            # Intentionally omit base_uom — IA1 should reject this
            result = ia_page.create_item_attribute(data)
            assert result['status'] != 'PASSED', \
                "IA1: Form should fail when Name is filled but Base UOM is not selected"
        except Exception:
            raise
        finally:
            ia_page._cleanup()
    ...
    def test_blank_name_spaces_only(self, ia_page):
        """IA1-C03: Name with only spaces -> Should be rejected (BUG-IA06)."""
        try:
    ...
    def test_duplicate_name_create(self, ia_page):
        """IA1-C04: Duplicate Name in Create -> Should block (BUG-IA01)."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
    ...
    def test_special_chars_in_name(self, ia_page):
        """IA1-C05: Special characters in Name -> Should reject (BUG-IA07)."""
        try:
    ...
    def test_very_long_name(self, ia_page):
        """IA1-C06: 300 character Name -> Should reject (BUG-IA04)."""
        try:
    ...
    def test_create_valid_all_fields(self, ia_page):
        """IA1-C07: Create valid item attribute with Name + Base UOM + Description -> Success."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
            assert result['status'] == 'PASSED', \
    ...
    def test_create_with_special_char_description(self, ia_page):
        """IA1-C08: Create with special characters in Description -> Should be accepted."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
    ...
    def test_name_with_leading_trailing_spaces(self, ia_page):
        """IA1-C09: Name with leading/trailing spaces -> Should trim (BUG-IA06)."""
        try:
    ...
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_create_empty_base_uom(self, ia_page):
        """IA1-C10: IA1-specific — Fill Name but leave Base UOM unselected -> Validation Failed."""
        try:
            ia_page.open_add_form()
            ia_page._force_close_panels()
            # Fill only Name — deliberately skip Base UOM selection
            ia_page.fill_item_attribute_form({
                'name': generate_ia_name("NoUOM", ATTR_NUM),
                'description': None,
            })
            ia_page.submit()
            is_alert = ia_page.is_validation_alert_present(timeout=10)
            if is_alert:
                warning = ia_page.handle_validation_warning()
                assert VALIDATION_FAILED_TITLE in warning, \
                    f"Expected 'Validation Failed' for missing Base UOM, got: {warning}"
            else:
                assert False, \
                    "No validation alert appeared when Base UOM is empty on IA1"
        except Exception:
            raise
        finally:
            ia_page._cleanup()
...
class TestEditFormValidations:
    """IA1-E01 to IA1-E04: Edit form validation tests for IA1."""
    ...
    def test_edit_duplicate_name(self, ia_page):
        """IA1-E01: Edit to duplicate name -> Should block (BUG-IA01)."""
        try:
    ...
    def test_edit_blank_name(self, ia_page):
        """IA1-E02: Edit Name to blank (spaces only) -> Should reject (BUG-IA06)."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
    ...
    def test_edit_pre_populated_fields(self, ia_page):
        """IA1-E03: Edit popup shows pre-filled data."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_edit_valid_update(self, ia_page):
        """IA1-E04: Edit with valid new Name and Description -> Success."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
...
class TestSearchFilter:
    """IA1-S01 to IA1-S04: Search and filter tests for IA1."""

    @pytest.mark.smoke
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_search_exact_match(self, ia_page):
        """IA1-S01: Search with exact Name -> Find the item."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
    ...
    def test_search_partial_match(self, ia_page):
        """IA1-S02: Search with partial Name -> Find matching items."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
            assert result['status'] == 'PASSED', f"Create failed: {result['error']}"
    ...
    def test_search_nonexistent(self, ia_page):
        """IA1-S03: Search for non-existent Name -> No results."""
        try:
    ...
    def test_search_clear_restores_table(self, ia_page):
        """IA1-S04: After searching, clearing should restore full table."""
        try:
...
class TestPopupUIBehaviors:
    """IA1-P01 to IA1-P03: Popup UI behavior tests for IA1."""

    @pytest.mark.ui
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_cancel_discards_data(self, ia_page):
        """IA1-P01: Cancel on Add form discards data -> Not saved in table."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            ia_page.open_add_form()
    ...
    def test_view_shows_read_only(self, ia_page):
        """IA1-P02: View popup fields are read-only."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_edit_has_update_button(self, ia_page):
        """IA1-P03: Edit popup shows Update button."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
...
# ╔══════════════════════════════════════════╗
# ║  PHASE 5: History Validations (2)       ║
# ╚══════════════════════════════════════════╝

class TestHistoryValidations:
    """IA1-H01 to IA1-H02: History popup validation tests for IA1."""
    ...
    def test_history_after_create(self, ia_page):
        """IA1-H01: History after creation -> Should have 1+ rows (BUG-IA08)."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_history_after_edit(self, ia_page):
        """IA1-H02: History after edit shows entry."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
