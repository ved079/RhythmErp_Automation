...
_unique_data = lambda: _ud(attr_num=1)


@pytest.fixture(scope="function")
def ia_page(logged_in_page):
    p = IAPlaywrightPage(logged_in_page, attr_num=1)
    p.navigate_to_page()
    yield p
    try:
        p.close_popup()
...
class TestIA1UIGroup5:
    def test_duplicate_name_rejected(self, ia_page):
        ...
        data = {"name": existing_name, "description": "dup test", "base_uom": ""}
        ia_page.open_add_form()
