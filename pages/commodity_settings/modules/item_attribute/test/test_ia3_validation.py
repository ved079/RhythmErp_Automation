"""
Item Attribute 3 (IA3) — 23 Automated Test Cases

IA3 is a standard IA screen (no Base UOM).
Fields: Name (required), Description (optional), Status (toggle).

5 Test Classes:
  TestCreateFormValidations   (9 tests:  IA3-C01 to IA3-C09)
  TestEditFormValidations     (4 tests:  IA3-E01 to IA3-E04)
  TestSearchFilter            (4 tests:  IA3-S01 to IA3-S04)
  TestPopupUIBehaviors        (3 tests:  IA3-P01 to IA3-P03)
  TestHistoryValidations      (3 tests:  IA3-H01 to IA3-H03)

Pytest Marker Summary:
  smoke:      6 tests (critical path)
  sanity:     23 tests (core validation)
  regression: 23 tests (full suite)
  bug:        7 tests (known open bugs)
...
GOLDEN CODE pattern (UOM v2):
  ...
  - Hardcoded ATTR_NUM = 3 (no parametrization — enables xdist -n 5)
"""
...
class TestCreateFormValidations:
    """IA3-C01 to IA3-C09: Create form validation tests for IA3."""

    @pytest.mark.smoke
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_empty_form_submit(self, ia_page):
        """IA3-C01: Submit with all fields empty -> Validation Failed."""
        try:
    ...
    def test_only_name_filled(self, ia_page):
        """IA3-C02: Submit with only Name filled -> Should PASS (no Base UOM required)."""
        try:
            data = generate_name_only_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
            assert result['status'] == 'PASSED', \
                f"IA3: Name-only creation should pass (no Base UOM), but got: {result['error']}"
        except Exception:
            raise
        finally:
            ia_page._cleanup()
    ...
    def test_blank_name_spaces_only(self, ia_page):
        """IA3-C03: Name with only spaces -> Should be rejected (BUG-IA06)."""
        try:
    ...
    def test_duplicate_name_create(self, ia_page):
        """IA3-C04: Duplicate Name in Create -> Should block (BUG-IA01)."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
    ...
    def test_special_chars_in_name(self, ia_page):
        """IA3-C05: Special characters in Name -> Should reject (BUG-IA07)."""
        try:
    ...
    def test_very_long_name(self, ia_page):
        """IA3-C06: 300 character Name -> Should reject (BUG-IA04)."""
        try:
    ...
    def test_create_valid_all_fields(self, ia_page):
        """IA3-C07: Create valid item attribute with Name + Description -> Success."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
            assert result['status'] == 'PASSED', \
    ...
    def test_create_with_special_char_description(self, ia_page):
        """IA3-C08: Create with special characters in Description -> Should be accepted."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
    ...
    def test_name_with_leading_trailing_spaces(self, ia_page):
        """IA3-C09: Name with leading/trailing spaces -> Should trim (BUG-IA06)."""
        try:
...
class TestEditFormValidations:
    """IA3-E01 to IA3-E04: Edit form validation tests for IA3."""
    ...
    def test_edit_duplicate_name(self, ia_page):
        """IA3-E01: Edit to duplicate name -> Should block (BUG-IA01)."""
        try:
    ...
    def test_edit_blank_name(self, ia_page):
        """IA3-E02: Edit Name to blank (spaces only) -> Should reject (BUG-IA06)."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
    ...
    def test_edit_pre_populated_fields(self, ia_page):
        """IA3-E03: Edit popup shows pre-filled data."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_edit_valid_update(self, ia_page):
        """IA3-E04: Edit with valid new Name and Description -> Success."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
...
class TestSearchFilter:
    """IA3-S01 to IA3-S04: Search and filter tests for IA3."""

    @pytest.mark.smoke
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_search_exact_match(self, ia_page):
        """IA3-S01: Search with exact Name -> Find the item."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
    ...
    def test_search_partial_match(self, ia_page):
        """IA3-S02: Search with partial Name -> Find matching items."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            result = ia_page.create_item_attribute(data)
            assert result['status'] == 'PASSED', f"Create failed: {result['error']}"
    ...
    def test_search_nonexistent(self, ia_page):
        """IA3-S03: Search for non-existent Name -> No results."""
        try:
    ...
    def test_search_clear_restores_table(self, ia_page):
        """IA3-S04: After searching, clearing should restore full table."""
        try:
...
class TestPopupUIBehaviors:
    """IA3-P01 to IA3-P03: Popup UI behavior tests for IA3."""

    @pytest.mark.ui
    @pytest.mark.sanity
    @pytest.mark.regression
    def test_cancel_discards_data(self, ia_page):
        """IA3-P01: Cancel on Add form discards data -> Not saved in table."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            ia_page.open_add_form()
    ...
    def test_view_shows_read_only(self, ia_page):
        """IA3-P02: View popup fields are read-only."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_edit_has_update_button(self, ia_page):
        """IA3-P03: Edit popup shows Update button."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
...
class TestHistoryValidations:
    """IA3-H01 to IA3-H03: History popup validation tests for IA3."""
    ...
    def test_history_after_create(self, ia_page):
        """IA3-H01: History after creation -> Should have 1+ rows (BUG-IA08)."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_history_after_edit(self, ia_page):
        """IA3-H02: History after edit shows entry."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
    ...
    def test_history_close_button(self, ia_page):
        """IA3-H03: History popup close button works."""
        try:
            data = generate_valid_ia_data(ATTR_NUM)
            r = ia_page.create_item_attribute(data)
            assert r['status'] == 'PASSED', f"Create failed: {r['error']}"
