...
from pages.commodity_settings.modules.commodity_quality_parameter.cqp_playwright_page import CQPPlaywrightPage
...
def cqp_page(logged_in_page):
    p = CQPPlaywrightPage(logged_in_page)
    p.navigate_to_page()
    yield p
    try:
        p.force_close_popup()
