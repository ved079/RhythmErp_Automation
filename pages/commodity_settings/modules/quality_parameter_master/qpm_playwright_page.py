...
class QPMPlaywrightPage(BasePlaywrightPage):
    URL        = f"{BASE_URL}/#/dynamic-screens/Quality%20Parameter%20Master"
    ...
    def search_parameter(self, name: str):
        self.search_entry(name)

    def verify_parameter_exists(self, name: str):
        self.page.wait_for_selector(
            f"//td[contains(@class,'cdk-column-name') and contains(.,'{name}')]",
            timeout=5000,
        )

    def is_parameter_in_table(self, name: str) -> bool:
