"""
test_quality_parameter_master_perf.py — Performance benchmarks + Live CRUD tests.
...
from pages.commodity_settings.modules.quality_parameter_master.data.quality_parameter_master_data import (
    generate_quality_parameter_payloads, generate_batch_payloads,
)
...
class TestQualityParameterMasterPerformance:
    """Verify Quality Parameter Master payload generation meets speed benchmarks and live CRUD."""
    ...
    def test_batch_create_5_quality_parameter_masters(self, api_client):
        """Create, Read, and Update 5 Quality Parameter Master entries via live API."""
        ...
        for i, entry_id in enumerate(created_ids):
            ...
            current_val = detail.get("name", "") or ""
            update_payload = dict(detail)
            update_payload["name"] = current_val + "-UPD"
