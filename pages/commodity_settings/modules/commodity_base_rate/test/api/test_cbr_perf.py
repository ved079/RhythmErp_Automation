"""
test_cbr_perf.py — Performance benchmarks for Commodity Base Rate payload generation.
...
class TestCBRPerformance:
    """Verify Commodity Base Rate payload generation meets speed benchmarks."""

    def test_payload_generation_speed(self):
        """Single payload generation should complete in under 10ms."""
        start = time.perf_counter()
        for _ in range(100):
            generate_cbr_payloads(count=1)
    ...
    def test_batch_create_5_cbr(self, api_client):
        """Create 5 Commodity Base Rate entries via API and verify they are persisted."""
        pass
