"""
test_item_category_perf.py — Performance benchmarks + Live CRUD tests.
...
from pages.commodity_settings.modules.item_category.data.item_category_data import (
    generate_item_category_payloads, generate_batch_payloads,
)
...
class TestItemCategoryPerformance:
    """Verify Item Category payload generation meets speed benchmarks and live CRUD."""
    ...
    def test_batch_create_5_item_categories(self, api_client):
        """Create, Read, and Update 5 Item Category entries via live API."""
        
        payloads = generate_batch_payloads(count=5)
        ts = datetime.datetime.now().strftime("%H%M%S")
        for i, p in enumerate(payloads):
            p["item_code"] = f"{p['item_code']}{ts}{i}"
            p["item_description"] = f"{p['item_description']}-{ts}{i}"
        ...
        for i, entry_id in enumerate(created_ids):
            ...
            current_val = detail.get("item_description", "") or ""
            update_payload = dict(detail)
            update_payload["item_description"] = current_val + "-UPD"
