"""
test_item_category_perf.py — Performance benchmarks for Item Category payload generation.
...
class TestItemCategoryPerformance:
    """Verify Item Category payload generation meets speed benchmarks."""

    def test_payload_generation_speed(self):
        """Single payload generation should complete in under 10ms."""
        start = time.perf_counter()
        for _ in range(100):
            generate_item_category_payloads(count=1)
    ...
    def test_batch_create_5_item_categories(self, api_client):
        """Create 5 Item Categories via API and verify they are persisted."""
        pass
