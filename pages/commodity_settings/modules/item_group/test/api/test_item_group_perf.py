"""
test_item_group_perf.py — Performance benchmarks + Live CRUD tests.
...
from pages.commodity_settings.modules.item_group.data.item_group_data import (
    generate_item_group_payloads, generate_batch_payloads,
)
...
class TestItemGroupPerformance:
    """Verify Item Group payload generation meets speed benchmarks and live CRUD."""
    ...
    def test_batch_create_5_item_groups(self, api_client):
        """Create, Read, and Update 5 Item Group entries via live API."""
        
        payloads = generate_batch_payloads(count=5)
        ts = datetime.datetime.now().strftime("%H%M%S")
        for i, p in enumerate(payloads):
            p["code"] = f"{p['code']}{ts}{i}"
